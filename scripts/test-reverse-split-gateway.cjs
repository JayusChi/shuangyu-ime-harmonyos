...
const ui = load('entry/src/main/ets/domain/candidate/CandidateUiPolicy.ets', {
  ...
  '../keyboard/ConvenienceInput': load('entry/src/main/ets/domain/keyboard/ConvenienceInput.ets'),
...
const newDirectCodes = ['ohx', 'ojg', 'ojz', 'ohz', 'ofz', 'ofa', 'ovd', 'oyx'];
...
assert.equal(displayActions.length, 21);
...
console.log('PASS: all 21 display/feedback direct actions cross the native gateway; unapproved targets are rejected.');

for (const [rawInput, source, expected] of [
  ['ojz', 'functional', '[自适应字号]'],
  ['other', 'functional', '[键字12号]'],
  ['ojz', 'user', '[键字12号]']
]) {
  response = composition({ rawInput, candidates: [{ id: 'font-default', text: '[键字12号]',
    displayText: '[键字12号]', reading: rawInput, source }] });
  const candidate = gateway.processKey(1, 'z').candidates[0];
  assert.equal(candidate.displayText, expected);
  assert.equal(candidate.text, '[键字12号]');
}
console.log('PASS: legacy ojz default label describes adaptive fonts without changing protocol text.');

const shapeAction = directData.records.find(record => record.code === 'oix');
assert.ok(shapeAction);
response = composition({
  action: { type: 'DIRECT_CONTROL', formatId: shapeAction.action, text: shapeAction.target, cursorOffsetUtf16: 0 },
  compositionFinished: true
});
assert.equal(gateway.selectCandidate(1, 0).action.text, 'flypy-shape');
