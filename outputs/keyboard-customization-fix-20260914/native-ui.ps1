param(
  ...
  [ValidateSet('dump','tap','double','fill','screen')][string]$Action = 'dump',
...
if ($Action -ne 'dump') {
  ...
  $clickAction = if ($Action -eq 'double') { 'doubleClick' } else { 'click' }
  Invoke-Hdc shell uitest uiInput $clickAction $x $y | Out-Null
  if ($Action -eq 'fill') {
