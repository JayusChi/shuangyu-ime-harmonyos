(() => {
  ...
  function renderImages() {
    ...
    for (const slot of EditorIO.IMAGE_KEYS) {
      ...
      file.addEventListener('change', () => withBusy(async () => {
        ...
        const path = `assets/${slot}.${extension}`;
  ...
  function renderPreview() {
    ...
    for (const rowDef of state.layout.rows) {
      ...
      for (const key of rowDef.keys) {
        ...
        if (key.id === 'punctuation-comma' || key.id === 'punctuation-period') button.append(el('span', { class:'upper', 'aria-hidden':'true' }, key.id === 'punctuation-comma' ? '=' : '@'));
  ...
  async function withBusy(action) {
    ...
    finally { busy = false; controls.forEach(([control,disabled]) => control.disabled = disabled); syncInspector(); }
  }
  ...
  $('import-file').addEventListener('change', () => withBusy(async () => {
    ...
    else state.selected = state.layout.rows[0].keys[0].id;
