...
$deliveryDir=Join-Path $repoRoot 'artifacts/0.12.0'
...
$previous=Get-Content -LiteralPath (Join-Path $repoRoot 'outputs/release-0.11.0-clipboard-reverse/profile-verification.json') -Raw | ConvertFrom-Json
...
try {
 ...
 if($pack.summary.app.bundleName -ne 'com.corrosion.shuangyuime' -or $pack.summary.app.version.name -ne '0.12.0' -or $pack.summary.app.version.code -ne 12000000){throw 'APP version mismatch'}
...
try {
 ...
 if($module.app.versionName -ne '0.12.0' -or $module.app.versionCode -ne 12000000 -or $module.app.debug -or $module.app.buildMode -ne 'release'){throw 'Embedded HAP metadata mismatch'}
...
$appName='ShuangYuIME-0.12.0-12000000-release-signed.app'
$hapName='ShuangYuIME-0.12.0-12000000-release-signed.hap'
...
$manifest=[ordered]@{
 ...
 distribution=$profile.content.'app-distribution-type';sameReleaseCertificateAndAppIdAs0110=$true
