...
$deliveryDir=Join-Path $repoRoot 'artifacts/0.13.0'
...
& $java -jar $signTool verify-app -inFile $SignedHap -outCertChain (Join-Path $PSScriptRoot 'hap-certificate-chain.cer') -outProfile (Join-Path $PSScriptRoot 'hap-profile.p7b') *> (Join-Path $PSScriptRoot 'verify-hap-signature.log')
if($LASTEXITCODE -ne 0){throw 'Standalone HAP signature verification failed'}
if((Get-FileHash -LiteralPath (Join-Path $PSScriptRoot 'hap-profile.p7b')).Hash -cne (Get-FileHash -LiteralPath (Join-Path $PSScriptRoot 'app-profile.p7b')).Hash){throw 'APP/HAP signing profile mismatch'}
...
if(($profile.content | ConvertTo-Json -Depth 30 -Compress) -notmatch '"group\.1516753898738609081"'){throw 'Release profile lacks the configured customization shared group'}
...
$previous=Get-Content -LiteralPath (Join-Path $repoRoot 'outputs/release-0.12.0/profile-verification.json') -Raw | ConvertFrom-Json
...
try {
 ...
 if($pack.summary.app.bundleName -ne 'com.corrosion.shuangyuime' -or $pack.summary.app.version.name -ne '0.13.0' -or $pack.summary.app.version.code -ne 13000000){throw 'APP version mismatch'}
...
try {
 ...
 if($module.app.versionName -ne '0.13.0' -or $module.app.versionCode -ne 13000000 -or $module.app.debug -or $module.app.buildMode -ne 'release'){throw 'Embedded HAP metadata mismatch'}
 ...
 $imeExtension=@($module.module.extensionAbilities | Where-Object type -eq 'inputMethod')
 if($imeExtension.Count -ne 1 -or 'group.1516753898738609081' -notin $imeExtension[0].dataGroupIds){throw 'Packaged input method lacks customization shared group'}
} finally {$zip.Dispose()}
...
$appName='ShuangYuIME-0.13.0-13000000-release-signed.app'
$hapName='ShuangYuIME-0.13.0-13000000-release-signed.hap'
...
$manifest=[ordered]@{
 ...
 distribution=$profile.content.'app-distribution-type';sameReleaseCertificateAndAppIdAs0120=$true
 profileExpiry=[DateTimeOffset]::FromUnixTimeSeconds($profile.content.validity.'not-after').ToString('o')
 appSignature='PASS';profileSignature='PASS';standaloneHapSignature='PASS';signedHapGate='PASS';embeddedHapGate='PASS'
 ...
 customizationSharing='Configured shared group authorized by release profile; simulator import/render acceptance recorded separately.'
