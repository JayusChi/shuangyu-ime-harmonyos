...
## 2026-09-15（0.13.0 正式签名 APP）

- 版本升为 `0.13.0 / 13000000 / build 1`，基于当前功能源码打包，包含皮肤工坊入口及此前反馈修复。
- 双 ABI Rust 构建、clean Release HAP、APP 打包、签名和资源门禁通过；正式 Profile 的共享目录授权匹配，发布证书及应用标识与 0.12.0 一致。
- 交付见 `artifacts/0.13.0`，核查见 `outputs/release-0.13.0/RELEASE_READINESS.md`。功能验收沿用打包前的双端模拟器记录；本轮未安装 0.13.0 或上传 AGC。

## 2026-09-14（0.12.0 客户测试 APP）

- 版本升为 `0.12.0 / 12000000 / build 1`，包含当天手机、电脑模拟器验收通过的九项键盘、候选和双拼反馈修复。
- 完成 clean Release HAP、APP 打包及正式签名核验；与0.11.0发布证书、APP标识一致。与刚验收的HAP相比，46个模块文件只有版本元数据变化，ArkTS字节码、双ABI引擎及全部资源一致。
- APP/Profile/HAP签名、版本、权限和资源门禁通过，交付见 `artifacts/0.12.0/AGC_UPLOAD_README.md`。本轮仅更新版本并打包，功能验证沿用紧邻的748项ArkTS、31项UI证据及双拼专项326项Rust结果；未上传AGC或重新安装0.12.0。

## 2026-09-14（手机、电脑模拟器反馈复验）

- 九项键盘/候选/双拼反馈完成双端模拟器验收；实测补修慢速下滑回删被长按连删抢先触发的问题，并拦截候选清空后重建按键的尾随点击。
- ArkTS 748项、UI证据31项通过。正式HAP已重新构建，版本保持0.11.0；未重建客户APP。证据及设置恢复差异见 `docs/evidence/2026-09-14-feedback-reacceptance/README.md`。

## 2026-09-11（0.11.0 正式签名 APP）

- 版本更新为 `0.11.0 / 11000000 / build 1`，纳入下述应用内查形、键盘与双拼反馈修复；本轮产品源码只改版本元数据。
- 双 ABI Native、clean Release HAP、APP 构建与正式签名验证通过；发布证书/APP 标识与0.10.0一致，APP内嵌与独立HAP的版本、权限、网页路由及资源门禁通过。
- 交付见 `artifacts/0.11.0/AGC_UPLOAD_README.md`。未上传AGC，未重新安装0.11.0到模拟器或真机；功能验收沿用打包前刚完成的708项ArkTS与双端18项网页检查。

## 2026-09-11（查形完整网页补修）

- 经用户批准增加 INTERNET，并将 oix 光标/剪贴板查形及小鹤官网/帮助直通接入独立应用内网页，解决手机模拟器系统浏览器冷启动全屏空白的问题。
- 网页窗口支持重复查询、换字更新、刷新、返回、关闭及失败重试；精确校验小鹤 HTTPS 入口，使用非导出能力。
- 发布检查纳入明确的 `flypy-web-v1` 权限配置和最终 HAP 页面路由检查，继续拒绝未批准权限及资源。手机、电脑18项网页/宿主检查通过，最终源码708项 ArkTS 回归通过，两端已安全覆盖安装且46项设置保留。详见 `outputs/browser-fullscreen-fix-20260911/ACCEPTANCE_REPORT.md`。

## 2026-09-11（键盘反馈双模拟器验收）

- 手机与电脑模拟器复核8项键盘需求及两条双拼例句；实测修复触屏/鼠标候选控制器没有能力上下文，导致 oix、xhgw 等直通无法打开网页的问题。通过当前输入法生命周期共享启动器，并保护新旧能力替换。
- 修复异步查形结果已解析但固定候选仍显示 `{last_0}` 的问题；固定、浮动及展开候选按显示文字刷新。
- ArkTS 707、相关 Rust 374、工坊10项检查通过；最终设备开发签名 Release HAP 在 Phone/2in1 无重置安装，设置恢复。UI证据22项通过；手机浏览器全屏仍空白的3项保留未完成，标签预览能看到正确官网与查形结果。未测试真实实体键盘、未重打正式客户 APP。详见 `outputs/feedback-acceptance-20260911/ACCEPTANCE_REPORT.md`。

## 2026-09-10（0.10.0 客户测试包）

- 版本更新为 `0.10.0`（`versionCode=10000000`、`buildVersion=1`），纳入 0.9.0 交付之后的客户反馈修复及全部可达设置详情/子页 UI 统一。宽屏沿用左分类/右详情，窄屏逐级导航；UI 迁移的控制器、存储、模型、Native、Rust 与 43 个原业务方法复核保持。
- 新运行 ArkTS 686 项、共享存储契约 8 项通过，补齐上一轮 SDK Previewer 崩溃造成的测试缺口；双 ABI Native、clean release 产品 APP、正式签名/版本/资源门禁通过，发布证书与 APP 标识保持 0.9.0 身份。
- 同源码开发签名的 0.10.0 包已在手机与电脑模拟器无重置升级，版本、皮肤子页、返回及布局补测通过。平板沿用 9 月 9 日三端 UI 验收；正式 APP 未上传 AGC，客户渠道安装及真实宿主/实体键盘待测。
- 客户明确接受自定义结构/皮肤导入仍待华为共享沙箱授权；已在交付说明中注明，内置结构/皮肤可测。交付见 `artifacts/0.10.0/AGC_UPLOAD_README.md`，完整核查见 `outputs/release-0.10.0/RELEASE_READINESS.md`。

## 2026-09-09（虚拟键盘输入法选择列表修复）

- 修复 `ϟ12` 下方按键弹窗以键盘子类型为主要条目的问题：改为按系统已启用输入法应用名称展示，每个应用仅一项；点击后切换对应输入法应用，当前应用选择与取消均不改变键盘方案。
- 新增应用去重、目标选择、已禁用目标、关闭/重复请求、迟到查询与失败重试回归。ArkTS 645/645、Release 构建及发布资源门禁通过；Phone 模拟器验证“小艺/双羽”列表和实际切换到小艺，百度未安装未测。证据见 `docs/evidence/2026-09-09-input-method-app-picker/README.md`。
- 本轮未重打客户 APP，2026-09-08 的 0.9.0 交付 APP 不包含此修复。

## 2026-09-08（0.9.0 客户测试包）

- 应用版本升级为 `0.9.0`（`versionCode=9000000`），纳入小鹤音形三/四码自动切分、`oit` 模式切换、中文 `〔〕` 成对键及系统输入法子类型同步修复。
- 本机默认产品重新绑定已有的正式 `release` 签名配置，避免客户包使用开发证书和设备白名单；最终发布证书与 0.8.0 一致。
- 双 ABI Native、clean Release HAP、signed APP、APP/Profile 签名、版本/应用身份及 signed/APP 内嵌 HAP 发布内容门禁通过。交付文件位于 `artifacts/0.9.0/`，验证记录位于 `outputs/release-0.9.0/delivery-verification.json`。
- 本轮为版本与交付构建，未新增设备验收；功能验证沿用当天 Rust 654 项、ArkTS 639 项及既有模拟器记录，QQ、备忘录、头条和真实实体键盘场景仍需客户测试。

## 2026-09-04（0.8.0 客户测试包）

- 应用版本升级为 `0.8.0`（`versionCode=8000000`），纳入本轮实体键盘分号引导、虚拟键盘下滑符号、候选展开、智能标点、符号页、系统键和浮动候选的客户反馈闭环修复。
- 电脑模拟器最终验收、ArkTS 全量单元测试、Release 清理构建和发布内容门禁均已通过；验收记录位于 `outputs/emulator-acceptance-20260904/ACCEPTANCE_REPORT.md`。

## 2026-09-03（实体键盘分号引导功能动作直接执行修复）

- 修正 0.6.0 只自动提交静态快符、仍要求功能动作再按空格的错误实现：正式小鹤音形在 `;` 后输入单个字母并唯一精确命中时，普通符号直接提交，成对符号、重复、撤销和行末定位直接返回并执行封闭动作，不再停留在候选栏等待确认。
- 成对符号继续走原子插入服务，写入一次后按 UTF-16 偏移把光标放到中间；`;i` 的引导字母不再提前使刚上屏内容的撤销记录失效，`;f` 与 `;n` 分别复用既有重复上屏和当前行末定位实现。
- 自动执行仅适用于分号后的单字母唯一精确结果；多字母动作、同码多候选及单独分号的冒号候选仍保留显式候选确认。码表运行时 `83/83`、正式音形 `21/21`、FFI `39/39` 和 ArkTS 单元测试通过；本轮未执行实体键盘真机验收。
