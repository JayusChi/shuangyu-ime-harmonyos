...
## 2026-09-01（小鹤音形四码自动上屏、顶屏与清屏边界修复）

- 按客户最新规则取消空码正向/反向切分：四码空码且无有效后续时只按设置清屏或完整保留，不再把短码首选提交后重放尾码；`niuo`、`ladj` 不会再表现成四码内顶屏。
- 顶屏统一要求旧四码不存在有效更长编码；四码重码且无后续时，第五码提交旧段首选并作为新段首键保留，有后续编码时继续组成原段。
- 实体键盘浮动候选窗在组合码非空但候选暂为空时继续显示输入码与空码提示，`jda` 三码状态不再视觉消失。清码直通候选文案明确为“[四码空码清] / [空码不清]”。
- 新增正式词库 `niuo / ladj / jda`、空码清/不清、顶屏后续码和浮动窗空候选专项回归；码表运行时 `83/83`、码表协议 `24/24`、正式小鹤音形 `21/21`、FFI `39/39`、ArkTS 全量任务与 Rust 严格 Clippy 均 PASS。

## 2026-08-31（0.6.0 开发设备签名一致性修复）

- 定位 DevEco 部署错误 `9568332 / install sign info inconsistent`：在线 Phone/Pad 环境中的 0.5.1 使用旧 Debug 证书，而当前 `default` 产品已固定使用正式 Release 证书；包名和 APP ID 相同也不能跨证书覆盖。
- 新增 `scripts/install-signed-hap.ps1`，统一处理 Phone、Tablet、2in1/PC 的 signed HAP 安装、HDC 未授权提示、输入法启用/选中状态恢复，以及显式 `-ResetSignature` 的一次性证书迁移。默认模式绝不删除旧包数据；只有明确传入重置参数才会卸载旧签名应用。
- 实测确认 `bm uninstall -k` 会保留旧签名绑定，随后安装仍返回 `9568332`，因此跨证书迁移不能保留旧沙箱。两台在线 x86_64 Phone/Pad 开发环境的旧 0.5.1 沙箱已随证书重置清除，随后成功安装 0.6.0 固定 Release 身份，并通过同包 `bm install -r` 覆盖复验；原为当前输入法的环境已恢复 `FULL_EXPERIENCE_MODE` 和当前选择。受系统目录权限限制，旧沙箱及旧安装 HAP 未能导出，不能从本次迁移记录恢复。第三台设备仍等待设备侧 HDC 授权，未执行卸载或安装。

## 2026-08-31（0.6.0 客户反馈第 1～6 点实体键盘复核与浮动窗样式修复）

- 明确本轮客户反馈第 1～6 点均以实体键盘为验收入口，并逐项复核实际调用链：分号引导和唯一快符走实体字母串行队列；唯一候选顶屏、冒号候选/空格确认/双击分号走实体分号实时状态路由；分类 `#直` 由实体键盘共用的正式码表数据层直接加载。
- 按客户对第 5 点的补充说明，把 `5.直通.txt` 改为实体键盘直通动作词条来源，而不再只作留档或按编码固化在 Rust 中。当前客户文件 44 行中，31 行受支持语义被离线转换为类型化数据记录，编码、候选标题和顺序均取自原行；13 行因功能尚未实现或涉及 HTTP、动态搜索/剪贴板、旧平台键码等风险而明确进入转换报告。以后增删受支持行并重新导入即可生效，运行时仍不解释任意 `$cmd/$ddcmd`。
- 修复第 6 点：输入码与候选共同显示在浮动候选窗时，输入码不再绘制下划线；嵌入/固定展示位仍用实线下划线表达未上屏。样式由 `CompositionPreeditPlacement` 明确决定，不改变 `rawInput`、候选、查询和提交文本。
- 根据“第 4 点也是实体键盘问题”的澄清补修真实缺口：HarmonyOS PC 宿主可能先完整派发现代 API 的 DOWN/UP，再派发旧 API 的 DOWN/UP；旧去重器只保留最后事件，会漏掉前一 DOWN 并把同一次按键处理两遍。现在按 `(keyCode, DOWN/UP)` 保存 80 ms 有界决策并跨通道一次性配对，`o` 和后续 `k` 各只进入组合一次；同通道长按重复不受抑制。此前触屏键盘行身份修复继续保留为独立防护，但不再把它误记为本次实体键盘问题的根因。
- 新增实体双通道完整按下/抬起交错、`o → ok` 单次派发、浮动/嵌入装饰对照，以及生产直通数据拒绝原始命令/非授权目标的回归。ArkTS 全量、转换器 `24/24`、码表运行时 `104/104`、正式音形 `19/19`、Rust fmt/严格 Clippy及 x86_64/arm64-v8a OHOS Native 编译均 PASS。正式词库 bundle 内容未变化；本轮未执行设备安装和实体键盘实机视觉验收。

## 2026-08-31（0.6.0 客户反馈：分类直通改为可管理词条）

- 修复正式转换器把分类词库 `#直` 编译成普通新增并在 bundle 中抹掉标记的问题。现在任一受支持分类里的 `词条<Tab>编码#直` 或 `上屏内容,候选提示<Tab>编码#直` 都会原样进入独立用户规则层，保留所属分类、候选提示和 `Direct` 类型；不再逐词查找、搬家或固化成功能。
- 直通继续按“完整编码＋上屏内容”作为数据键：可在用户词库新增或覆盖同格式 `#直`，也可用同词同码 `#删` 隐藏包内直通。新增正式回归确认 `给予<Tab>gwyu#直` 的类型、前缀提示、干净上屏以及外部删除均生效；旧 bundle 的分类 `Add` 表示仅保留向后兼容读取。
- 修复客户导入器把 `5.直通.txt` 只当旧平台留档的问题。该文件现在独立生成受控直通动作数据表；分类内嵌 `#直` 仍留在原分类。原始 `$cmd/$ddcmd` 不进入运行时，只有已实现且通过白名单的语义会转换为候选动作，编码和候选文案不再在 Rust 中逐条固化。
- 导入器同时固化既有实体分号兼容转换，客户旧行会稳定生成 `_ → ：` 与 `; → ；`，后续回传不会覆盖 0.6.0 已修复行为。最终 bundle 含 12 分类、17 条有效快符、162,732 条接受记录、346 条安全拒绝、1 条直通和 36 条固顶；大小 `56,161,934` bytes，SHA-256 `CE91129203F82BB7547C91B4EEE82FD749BB5BF28D2F297132D8410434EB5000`，内容 SHA-256 `9BA48DA6CE44B8852FB5A37747C1C3AE88FCBFEE4C4C2FB4C632AB23B91D7443`。
- 来源审计、两次确定性构建、转换器 `24/24`、码表运行时 `103/103`、正式音形 `19/19`、Rust fmt/严格 Clippy 和 Release 资源输入门禁全部通过。本轮未执行设备安装与真机输入回归。

## 2026-08-31（0.6.0 客户反馈：`o` 起始编码重复修复）

- 修复触屏键盘输入 `o` 起始编码时末键偶发重复的问题：候选/组合状态刷新不再改变 ArkUI 键盘行身份，手指按下到抬起期间不会因整行组件重建而把同一次点击再次派发；键盘布局同时改为只在编辑器、模式、方案或自定义配置变化时重建。
- 锁定 `o` 前缀语义：单击 `o` 只查询 `o` 的精确候选，继续输入 `k` 只查询 `ok`，不会跳成 `oo/okk`；`o` 作为直通功能码公共前缀、`ok` 作为 OK 拼字前缀的继续输入能力保持不变。
- 新增正式音形专项回归，逐键核对 `o → ok` 的 `rawInput` 与精确候选，并验证 `ocd` 仍可触发“设置菜单”直通动作；正式音形引擎 `19/19`、ArkTS `574/574` 通过。正式 bundle 内容未变化，仍为 `56,184,505` bytes、SHA-256 `EDEDA1CF055CB959B01F26683A7FC850737A061E0E6D12420A043B4C2A670C60`。本轮未执行设备触屏实机回归。

## 2026-08-31（0.6.0 客户反馈：实体键盘分号修复）

- 修复实体键盘输入“分号引导＋字母”后仍需额外按空格的问题：唯一精确的静态快符现在随字母直接上屏；撤销、重复、成对符号等功能动作仍保留候选确认，避免误触。
- 修复快速连续输入时分号按旧候选快照路由、导致唯一候选不能顶首选的问题：小鹤音形实体分号改为进入串行队列后按实时组合判定；候选不少于 2 个时选择次选，只有 1 个时先提交首选再进入快符引导，无候选时直接进入引导。
- 修正分号自身合同：单击分号显示冒号候选，按空格上屏冒号，连续按两次分号上屏全角分号；正式快符源恢复 `_ → ：` 与 `; → ；` 两个保留项。
- 正式音形 bundle 快符由 16 条增至 17 条，重建后为 `56,184,505` bytes、SHA-256 `EDEDA1CF055CB959B01F26683A7FC850737A061E0E6D12420A043B4C2A670C60`；源审计、转换器 24 项、码表运行时 103 项、正式音形引擎 18 项、严格 Clippy、ArkTS `574/574` 及 Release 资源输入门禁均通过。本轮未执行设备安装与实体键盘实机回归。

## 2026-08-28（0.6.0 客户测试包与安装图标修复）

- 应用版本升级为 `0.6.0`（`versionCode=6000000`）。
- 将 AppGallery Connect 使用的白色环形双羽图标同步写入应用级、Entry/输入法模块和启动窗口资源；安装包不再继续携带 0.4.0 起沿用的“翅膀＋键帽”旧图标。
- 首次 0.6.0 候选误用了 DevEco 自动生成的调试 Profile，虽能通过本地密码学验签，但 AGC 解析返回错误码 `993`。现已切回 0.5.0 成功发布时保存的正式发布材料；最终 APP/HAP 内嵌 Profile 均为 `type=release`，包名为 `com.corrosion.shuangyuime`，APP ID 为 `6917611076350696172`，无调试设备白名单，有效期至 2029-07-17。
- 双 ABI Native、clean Release HAP 与 signed APP 构建成功；Release 资源输入门禁、unsigned/signed/APP 内嵌 HAP 内容门禁和 APP 签名复核全部 PASS。unsigned HAP 为 `92,637,226` bytes、SHA-256 `A090FC2D2DFF13B6C1927AEB28366BBC1CF1A9B30738C6B2313B50BA39B23426`；最终 signed HAP 为 `92,800,195` bytes、SHA-256 `17732794B8EE202456029F453E987936AC93738AB049EB7AF381C0368AE8735E`；最终 signed APP 为 `24,741,084` bytes、SHA-256 `DD27260C3068139D5055A491E725F29D1808B20D8AD0178BECAA4AC8B35BB80A`。本轮未执行设备安装与功能回归。

## 2026-08-27（0.5.1 客户反馈闭环修复）

- 应用补丁版本升级为 `0.5.1`（`versionCode=5001000`）。
- 修复复制完成后选区不退出、紧接粘贴看似无效的问题：复制命令派发后保留 500 ms 有界窗口供异步宿主读取选区，再优先使用编辑器右移命令按原生语义收起选区，仅在命令失败时回退零宽范围选择。针对 HarmonyOS 6.1 独立宿主拒绝扩展粘贴动作读取系统剪贴板的情况，皮肤“全选/复制”同步保存有界进程内文本，随后皮肤粘贴直接提交该文本；未经过本键盘复制时仍回退宿主原生粘贴动作。
- 修复回删下滑“删除当前行”和回车下滑“恢复删除”在部分宿主中无效的问题：删行改为围绕光标分两侧删除，不再依赖宿主未实现的范围选择；中途失败会回写已删右半行，成功后继续保存带上下文的安全恢复记录。
- 修正输入码展示合同：所有中文组合码只在输入法候选区域自绘，并以实线下划线表达未上屏；不再调用无法保证下划线样式的宿主 `TextPreview`，从而同时消除候选栏/光标重复、编码跟随提交和无下划线三类差异。
- 修复小鹤音形万能键的最后一层展示缺陷：Rust/FFI 返回的反查候选不再被普通精确码 UI 过滤器二次隐藏；补齐正式 bundle 跨 FFI 及 ArkTS 万能码候选可见性回归，验证 `ju` + 反引号返回完整反查候选。ArkTS → Native 网关同时加入仅记录成功状态、原始码长度与候选数的脱敏诊断，便于设备链路复验。
- 修复发布资源负向门禁的测试夹具，使其包含当前要求的输入法资源、模块声明与 Ability 基类，不再因夹具落后产生假失败。
- 最终主机与发布门禁：完整 Rust workspace PASS（FFI `39/39`），发布资源正负门禁 PASS；最新 ArkTS `567` 项测试源码和应用 Release 均编译通过。Windows Hypium Previewer 在开始执行最新用例前崩溃于 AMD OpenGL 驱动，未生成虚假的 `567/567` 报告；本轮新增用例之前的最近一次完整报告为 `565/565 PASS`，新增万能键、复制粘贴与删行恢复路径均已由 Pad 实机覆盖。
- 最终 Release 实包审计 PASS：unsigned HAP `71,296,141` bytes，SHA-256 `EA94F3F1776CCC7BE991A506EB672C4473370C0B411D0A4FD70BEAE1957E8A83`；signed HAP `71,450,631` bytes，SHA-256 `230536B8189C9BA7F3D7255BD1734D3743A61953FD0CE1828616C68A36E8A840`。
- 三设备复验完成：2in1 清洁配置 Stage 4 全项 PASS；Pad 上 `ju\`` 万能键候选、候选区下划线且宿主无原始码、`abc → 全选 → 复制 → 粘贴 = abcabc`、`a\nb → 删行 → 恢复` 均 PASS；ARM64 Phone 安装同版 signed HAP，并在系统浏览器确认原始码只在输入法候选区、宿主输入框保持空白。证据见 `docs/evidence/2026-08-27-v0.5.1-customer-feedback/README.md`。

## 2026-08-27（可选架高层与辅键盘统一）

- 皮肤键盘新增 46vp 架高层：左侧切换系统输入法，右侧隐藏键盘，中间集中中英文、符号和键盘方案入口；候选栏不再重复显示中英文切换，关闭架高层后通过空格长按或上滑切换。
- 设置页新增“键盘架高层”开关并实时调整输入面板高度。默认开启，手机已有系统底栏时可自由关闭，横屏 Pad 与电脑端虚拟键盘可按需开启。
- 数字与符号入口合并为 `ϟ12`：点击进入数字键盘、长按进入符号键盘；符号菜单同时保留在架高层。
- 数字、符号辅键盘的返回键统一放在左下；逗号与句号分别位于空格左右。
- 中文句号键在候选充足时选择第二候选，逗号键选择第三候选；目标候选不足时先上屏首选再输入标点，无组合时直接输入标点。
- 设置 schema 升级至 14；旧皮肤结构中的符号键和逗号/句号组合键在运行时自动兼容新布局。

## 2026-08-27（皮肤键盘上下滑编辑手势）

- 回删键新增上滑撤销最近上屏、下滑删除当前行；回车键新增上滑重复最近上屏、下滑恢复最近一次回删或删行内容，全部动作继续进入既有串行输入队列。
- 最近删除恢复会记录删除文字、光标位置及删除点两侧上下文；只有输入会话、光标和周边文本仍匹配时才恢复，切换输入框或发生其他直接上屏后自动失效，避免把旧内容写入错误位置。
- 26 键字母键新增自定义下滑符号；中文、英文使用两套独立映射，设置入口位于“键盘结构与皮肤”，键面不显示下滑符号提示，未设置的字母保持原行为。
- 设置 schema 升至 `13`，新增中英文下滑映射的校验、迁移、跨进程同步和持久化；补齐双向滑动互斥、动作绑定、删文恢复与设置往返回归。

## 2026-08-27（皮肤上档、万能键与复制粘贴修复）

- 修复皮肤上档字符复用普通中文标点通道、导致键帽与实际输出不一致的问题：数字及 `*`、`/`、`;`、`!`、`-`、`#`、`+`、`@`、反引号和单引号现按键帽原样输出，不再转换为中文/全角标点，分号上档也不会误入快符引导。
- 修复复制后仍保持选中态、紧接着粘贴会用相同内容覆盖原选区而看似无效的问题：复制与粘贴继续使用 HarmonyOS 编辑器内置命令，复制完成后将选区折叠到当前光标，使复制状态明确且后续粘贴可见。
- 核对并锁定皮肤 `Z` 键万能键动作到小鹤音形反查引擎的完整链路；新增上档原样输出、万能键反查、复制后折叠选区及继续粘贴的回归测试。

## 2026-08-27（直通词条用户词库管理修复）

- 修复外部用户词库把 `#直` 当成未知标记、导致直通词条只能随正式包内置而不能自行设定及加删的问题；现支持 `上屏内容<Tab>完整编码#直` 和 `上屏内容,候选提示<Tab>完整编码#直`。
- 用户词库结构化管理文档新增 `DIRECT` 动作和 `displayText`；设置页可新增直通词条、填写可选候选提示、批量转换为直通/其他动作、移除，并完整支持导入、导出、revision 原子保存与运行时热重载。
- 精准前缀查询会把用户直通提示稳定放在普通长码提示之前，完整编码仍只提交上屏内容；直通词条继续不进入万能键枚举。旧正式包内以分类归属记录的直通词条保持兼容。
- 新增解析拒绝、规范化往返、覆盖层动作、前缀优先级、Native JSON 和 ArkTS 编辑器序列化回归。

## 2026-08-27（PC 应用候选提交与编码跟随上屏修复）

- 修复 PC 兼容应用候选提交时未同步宿主光标选区、导致候选可见但中文无法写入的问题：候选栏模式现在先建立零宽光标锚点，再执行候选提交。
- 兼容部分 PC 编辑器“文字已经写入但 `insertText` 回执为 `false`”的行为；提交结果会核对光标前的实际文字，已落入文档时只确认一次，不重复写入。
- 原始输入码继续只保留在候选区域，提交链只向编辑器写入选中的候选文字；完整提交和部分提交都不会把编码拼接到中文后面跟随上屏。
- 新增 PC 选区锚定、错误回执实写确认和“编辑器内无原始编码”回归用例。

## 2026-08-27（输入码候选栏与光标互斥显示修复）

- 修复组合输入码同时出现在候选栏和编辑器光标处的重复显示；本条最初允许宿主 `TextPreview` 与候选栏二选一，最终行为已由上方 `0.5.1` 条目收紧为“始终只在候选区域显示并带实线下划线”。
- 原始编码不以普通文本或系统预览写入编辑器；候选提交、退格、清空和部分候选提交后的剩余编码只更新输入法自绘区域，不污染宿主文本。
- `CANDIDATE_BAR` 组合文本模式及互斥展示状态回归锁定“同一输入码任一时刻只显示一处”，并覆盖固定候选栏、展开候选区与电脑浮动候选窗。

## 2026-08-27（智能标点可选范围修复）

- 修复“智能标点”被硬编码成“智能句号”的问题：双击同一个已启用的中文标点时，会核对并替换为对应的单个英文标点，不再只支持 `。` → `.`。
- 新增可多选的“智能标点范围”设置，默认对应客户配置 `intelligsymbol=,./;\\!:?"$()[]^_<>`；任意标点可单独开启或关闭，也支持全部取消。
- 虚拟键盘和实体键盘共用相同的标点映射、时间窗和光标前文本核对合同；不同标点连按不会误合并。
- 设置 schema 升至 `12`，新增 `smartPunctuationSymbols` 持久化与校验；旧版 `smartPeriodTimeoutMs` 自动迁移并继续作为智能标点时间窗，不丢失用户原设置。

## 2026-08-26（可配置直通提示与上屏分离）

- 新增 `上屏内容,候选提示<Tab>完整编码#直` 语法，并落地客户词条 `给予,给ʲⁱ̌予<Tab>gwyu#直`：输入 `gwy` 时候选显示 `给ʲⁱ̌予u`，选中或输入完整 `gwyu` 后上屏 `给予`。
- `#直` 不再限于全码词文件；记录保留来源分类并随该分类开关启用或隐藏，精准前缀提示优先于普通长码提示，万能键仍不枚举直通记录。
- Native 候选协议升级至 interface/ABI 11，增加独立 `displayText`，实际提交继续只使用 `text`；Phone、Pad 和悬浮候选的展示及无障碍文案同步处理。
- 正式 bundle 更新为 56,184,164 bytes，SHA-256 `F7BBFDF4473E9317D618C9AD02A792B47FF2DAB8BFD74FA23416579E01F9BDC7`，并以 v5 资源收据替换应用内置包。

## 2026-08-21（分类词库置顶与直通语义）

- 按客户补充说明保持正式 11 个分类不变：快符继续只由分号引导查询，不进入普通候选或万能键结果。
- 全码词来源新增 `编码#直` 语法；转换器将其放入万能键不可见的规则层，完整编码仍可正常输入，并拒绝其他分类误用 `#直`。新增 `examples/小鹤音形全码词词库模板.txt`。
- 修正包内 36 条 `#固` 原先脱离全码词开关的问题：默认关闭全码词时不再泄漏这些置顶词，启用后原子恢复；外部用户词库继续独立于系统分类。
- 客户现有权威全码词源尚未标记 `#直`，因此本次只交付语法和运行能力，不擅自猜测具体直通词条；原始 `$cmd/$ddcmd`、网络和平台动作继续隔离。
- 转换器 23 项、码表运行时 95 项、输入引擎 158 项，共 276 项 Rust 测试、严格 Clippy 以及 ArkTS 466/466 通过。正式 bundle 重建/校验保持 11 分类、26,039,684 bytes 和 SHA-256 `CDA61BC4011AB03100B52327325A4C908C3AF1EDDFE3DAF3D2B871F840B15E94` 不变；x86_64/arm64-v8a Native Release 与 default Release HAP 构建通过，Release 内容门禁 PASS。unsigned HAP 为 40,671,548 bytes、SHA-256 `D1CB5173A27B90FB9C4B03AD9B0D916755ADA0947321FA4CCFF9A6BA403D4111`；设备验收未运行。

## 2026-08-20（小鹤音形四类默认关闭）

- 按客户要求，在完整直通语法尚未支持期间，将二简次选、全码词、生僻字、全码字改为默认不启用；分类仍保留在正式包和设置页中，需要时可手动开启。
- ArkTS 新装、设置重置、分类页“重置为默认”和 Rust 正式 bundle 使用同一七类默认集合。分类设置 schema 升至 `4`；升级时替换旧版“全部开启”快照，非全开自定义组合保持不变。
- 转换合同 SHA-256 更新为 `624AD86AB5B2702FD49B9CD1ACAC498706EE0F74411EC111D6D1B9A1DAD7DECF`；正式 bundle 为 26,039,684 bytes、SHA-256 `CDA61BC4011AB03100B52327325A4C908C3AF1EDDFE3DAF3D2B871F840B15E94`。
- 正式包重建/校验、基线生成、Rust runtime/engine/FFI/converter、双 ABI OHOS Release Native 和 default debug HAP 均通过；ArkTS 单测编译通过，但 Windows Hypium 宿主执行器连续两次在启动用例后无报告挂起，未记作全量单测 PASS。

## 2026-08-20（智能句号）

- 中文模式下在可配置时间窗内连续输入两个“。”，会将光标前已核对的中文句号安全替换为单个英文句点“.”；超时保留“。。”，光标前文本不匹配时也不会删除其他内容。
- 虚拟键盘与实体键盘共用同一替换合同；按键到达时间在进入串行输入队列时捕获，不受候选提交延迟影响。实体键盘的预编辑后缀计数同步按替换后的实际字符数维护。
- 设置 schema 升至 `8`，新增“智能句号”时间参数：关闭、300/500/800/1000 毫秒，默认 500 毫秒；持久化、跨进程同步、旧设置迁移和非法值回退均已覆盖。

## 2026-08-20（小鹤音形快符单击候选与双击分号）

- 快符转换器正式接收客户表中的两个保留码：`_` 表示单击分号时显示的默认候选，`;` 表示再次按分号时直接上屏的内容；这两个保留码只允许出现在 `quick-symbol` 分类。
- 小鹤音形单击分号现显示默认冒号候选，双击分号直接上屏全角分号；触屏、中文符号页和实体键盘统一走 Rust 引导状态机，快符分类关闭时不泄漏默认项。
- 正式快符分类由 15 条增至 17 条；叠加本日分类默认值调整后的最终 bundle 为 26,039,684 bytes、SHA-256 `CDA61BC4011AB03100B52327325A4C908C3AF1EDDFE3DAF3D2B871F840B15E94`，安装缓存沿用 `production-v2` 并通过绑定新哈希的校验回执自动刷新；新增 `examples/小鹤音形快符词库模板.txt`。

## 2026-08-20（电脑端空码后续编码提示修复）

- 电脑浮动候选窗改用与手机候选栏相同的精准匹配展示模型；输入 `nam`、`un` 等无精确项的前缀码时，候选文字后会显示剩余编码提示。
- 浮窗宽度估算同步计入后续编码，避免提示末尾被裁切；平板候选展示复用同一规则，并统一精准提示的无障碍播报。
- 按客户最终反馈，空码后续提示改为固定只保留源序第 1 项；引擎与手机、平板、电脑浮窗同步收紧，并移除原 1～9 项数量设置、持久化字段和控制器入口；精确同码重码不受影响。
- ArkTS 全量单测 `454/454` 通过；正式小鹤音形 `un` 源序专项通过，首候选保持“熟能生巧”（完整码 `unuq`）。

## 2026-08-25（0.5.0 客户测试版）

- 正式应用版本升级为 `0.5.0`（`versionCode=5000000`）。
- 限制横屏 T9 固定面板高度并停止失败后的无界 resize 重试，修复 Pad/2in1 冷启动首次不显示。
- 保留实体键盘候选提交后的智能句号窗口，使第二个句点由输入法消费并将 `。` 替换为 `.`。
- 修复从拼音方案热切换到“26 键小鹤音形”时误判码表已加载、原生引擎拒绝切换并回滚的问题；Phone、Pad、2in1 均已通过 Release 包复验。
- 按客户回填的 44 条直通确认表实现 31 条功能项，并保留 1 条已确认的 HTTP 安全拒绝：日期/农历/时间、固定小鹤 HTTPS 页面、设置与用户词库入口、删行、简繁/标点/全半角、数字句点、空码阈值、四码策略、三组词库预设、二简次选、用户词库导入和多行《静夜思》。6 条“不需要”及 5 条未确认项未实现。
- `ofi` 的“读取已复制单字但不粘贴”在 normal APL 应用上受 `ohos.permission.READ_PASTEBOARD` 的 `system_basic` 权限等级限制；实机验证失败后已撤下候选和权限声明，避免交付无效入口，等待客户选择显式粘贴/当前输入替代方案或提供系统级签名条件。
- 最终 signed Release 为 71,255,985 bytes、SHA-256 `39DA69D38C4BD5F0BCFE9358F8E2487BCFF6DF2A766DAD4581345DDF1749B4E9`；ArkTS、Rust、双 ABI 构建、Release 正负门禁及 Phone/Pad/2in1 代表性直通上屏均通过。
