...
> 历史记录：本页记录的是旧版 Phone 独立辅助行方案。2026-07-21 后续产品决策已取消该行，改为“中/英”键长按系统选择器，并只在 Pad、特殊窗口或系统输入法区域缺失时保留内联显式切换键；当前规则见 `docs/features/input-method/INPUT_METHOD_SWITCHER_CAPABILITY.md`。
