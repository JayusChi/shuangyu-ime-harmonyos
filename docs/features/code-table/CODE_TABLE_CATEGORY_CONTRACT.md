...
分类来源中的 `#固` 和 `#直` 属于该来源分类自身，不属于外部用户词库：`#固` 在普通候选前建立不可跨越的置顶保护；`#直` 可用 `上屏内容,候选提示<TAB>完整编码#直` 分离显示与提交文本，在不完整前缀下显示提示及剩余编码，选中或输完完整编码后只提交上屏内容，并且不参加反引号万能键反查。两者都随所属分类开关启用或隐藏。外部用户词库仍保持独立，不受系统分类开关影响。
