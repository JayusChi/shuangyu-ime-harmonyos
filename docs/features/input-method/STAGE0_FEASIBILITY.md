...
- Historical stage-0 note: the emulator was temporarily made visible by removing a malformed input-method profile. This workaround was superseded on 2026-08-26: the extension now declares standard `ohos.extension.input_method` metadata and a valid root-level `subtypes` profile. The profile was expanded on 2026-09-04 to expose 全拼、双拼、音形、英文 as four system-switchable subtypes.
