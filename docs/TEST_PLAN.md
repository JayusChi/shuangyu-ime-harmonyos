...
## 小鹤双拼整句与自定义二字简拼（2026-09-14）

- 排序：`hfyzxiwh` 和显式分音版本首选“很有希望”；测试词库不包含整句词条，低频三字词不能凭词长压过常用词组合；“期间／其间”维持频率顺序。
- 固定简拼：`双拼<Tab>up#固` 覆盖句首、句中、句尾、重复、分音边界、部分选词和退格。不能被高频系统长词跨越，正常声韵、跨两码边界和全拼方案不误替换；非固顶、非二字规则不注入句内，同码冲突保持源顺序。
- 管理：通过实际页面新增、保存，重启后规则保留；引擎中修改、删除和热重载及时更新当前组合；验收后移除新增词条并恢复原设置。
- 结果：相关 Rust 326/326、ArkTS 744/744、rustfmt、严格 Clippy、双 ABI Native 和正式签名 Release HAP 门禁 PASS。手机模拟器的虚拟触键与实体键盘事件两路径均完成“很有希望”“我用双拼输入法”首选与上屏；47 项设置恢复一致。真实外设和客户宿主未新增实测。详见 `docs/evidence/2026-09-14-shuangpin-fixed-words/README.md`。

## 浮动候选尺寸与阴影（2026-09-14）

- 同步：人为阻塞系统 resize，候选保持旧展示直到新几何就绪；连续输入只发布最新编码；清空/换会话后不重新显示旧内容，旧候选点击不能提交新候选。
- 尺寸：虚拟与实体模式共同覆盖增长、回删、候选高亮、光标移动、清空后重输、字号调整、候选数量变化和展开/收起。相同布局的行高稳定，可见组合内宽度不缩小，无多余 resize。
- 外观：白色圆角卡片、灰色序号及选中项；深色对比清晰；阴影按屏幕密度换算并预留透明空间，不出现滚动条或文字截断。
- 结果：744/744 ArkTS PASS；1320×2856 手机模拟器最终开发签名 Release 页面中，两模式 `k→kn→回删到k` 卡片高均为196 px，宽度718→812→812 px；选词上屏、硬件回车编码、深色实图及47项设置恢复通过。正式 release HAP 构建与资源门禁 PASS。真实外设、客户宿主及平板/电脑本轮实测仍待执行，详见 `docs/evidence/2026-09-14-floating-candidate/README.md`。

## 键盘字号、回车编码与实体翻页（2026-09-14）

- 字号：同逻辑尺寸的不同分辨率手机使用相同默认值，平板默认放大；旋转、增减、恢复默认、旧设置迁移和保存失败回滚。
- 回车：固定／浮动候选、实体主回车／小键盘回车／虚拟点按均上屏原始编码，有候选、无候选、引导码、分音符、九键数字码和功能候选均覆盖；不同时换行、搜索或发送，不执行功能候选。空组合恢复宿主回车动作，空格仍提交候选。
- 提交事务：编辑器拒绝或抛错后保留编码可重试；输入会话切换不清新状态；已上屏后的引擎清理失败不得重复提交编码；后续输入从新组合开始。
- 实体翻页：`[` 上一页、`]` 下一页；固定／浮动候选、未展开面板、虚拟键盘隐藏、快速输入后立即翻页、页首／页尾、无 Unicode 的键码驱动及 DOWN/UP 配对。Shift＋方括号保留花括号，Ctrl/Alt/Logo 和英文／受限编辑器保持原有路由。
- 当前主机回归：737/737 PASS（本轮新增 10 项）。既有候选下滑回删和右边距测试一并通过。设备视觉与真实 USB／蓝牙键盘仍需独立验收。

## 逆切分模式（2026-09-07）

默认传统与旧设置迁移；设置开关/`oit` 双向切换、重启和方案恢复；四码正常命中优先；缺任意二简时原有空码处理；前组唯一或重码、后组唯一或重码；符号二简；按候选页数量展示与分页选词；选后组次选提交完整组合；第五码顶前后首选且新码不丢失；退格恢复；用户排序/删除与分类开关；Native 动作校验；存储返回失败/抛异常均回滚。规范与设备验收步骤见 [逆切分模式](features/code-table/REVERSE_SPLIT_MODE.md)。
...
- 0.6.0 实体键盘反馈：按实体键盘入口连续验证 `;＋字母` 的唯一普通快符直接上屏，成对符号直接插入且光标居中，`;f` 立即重复、`;i` 立即撤销刚上屏内容、`;n` 立即移到当前行末尾；同时验证唯一候选后分号顶首选并进入引导、单分号冒号候选/空格冒号/双分号全角分号、双事件通道下 `o→ok` 不重复、分类 `#直/#删`、`5.直通.txt` 生成的动作候选，以及浮动窗输入码无下划线。不得用触屏点击用例代替这些实体键盘路由断言。
...
- 日志不输出正文。2026-09-11 用户批准应用内小鹤网页使用 INTERNET；Release 仅接受 `flypy-web-v1` 的 VIBRATE + INTERNET 权限组合及非导出的 FlypyWebAbility，继续拒绝 MICROPHONE、其他权限和缺少配置的联网声明。正式资源白名单不变。详见 `features/direct-control/FLYPY_WEB_PERMISSION_PROFILE.md`。
