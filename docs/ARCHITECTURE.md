...
性能修订后，解析器仍可返回最多 32 条有界展示路径，但候选热路径不再全部解码：raw input 不超过 12 字母时最多解码 4 条，超过 12 字母只解码首选路径。`quanpin` 不因组合长度、候选唯一性或下一按键自动提交，只有候选选择、空格、回车或标点等明确用户动作才能产生 `commitText`；64 字母是拒绝继续增长并保留现有组合的硬门禁。ArkTS 为支持能力的 `quanpin` 编辑器使用原生 TextPreview；不支持时原始编码只保留在候选区，不写入编辑器，并在等待编辑器异步确认前先发布同步候选结果，失败时回滚；C++ 仍仅转发协议。
...
ArkTS 只负责把结构化词条序列化为公开文本格式，不实施格式校验或候选硬排序。Rust 返回结构化管理文档并拥有解析校验、revision、恢复、保存和不可变快照替换。`#直` 在外部用户词库中保留独立 `Direct` 动作及可选候选提示，因而能复用同一条 CRUD、导入导出和热重载链。Common Event 只携带“文件已变化”的通知，限定本包 publisher，不携带词条正文。快捷键和命令行不直接调用 Native，从而避免产生第二套方案/分类状态。
...
浏览器等明确声明不支持文本预览的编辑器不会调用预览 API，而是进入 `CANDIDATE_BAR` 模式；组合输入码只存在于 InputSessionStore 和输入法自绘候选区域。候选确认时单次插入候选文字，取消、退格和清空只更新组合状态，不选择、删除或重写宿主编辑器文本。若宿主运行时明确拒绝 `setPreviewText`，连接层也切换到同一回退模式。该降级只改变输入码的展示位置和连接层所有权，不改变 Rust 解析、候选查询、候选确认和学习链路。

所有预览/候选栏组合、提交、清理和附着检查均绑定 `InputSessionStore` 的单调 generation。输入停止、键盘隐藏、编辑框切换或 Ability 销毁会使旧 generation 失效并重置连接状态；随后完成的旧异步操作只能返回陈旧结果，不能更新新会话。
...
人工规则不使用魔法分数。普通和直通用户候选使用独立 ID 命名空间并且 `learning_key=None`；`#直` 精确提交使用上屏文本，精准前缀提示使用可选展示文本且不会进入万能键枚举。系统候选被 `#固/#N` 提升时仍可保留原稳定 ID，但硬位置在每次软排序之后重新施加，因此学习无法突破。用户词库只在加载/显式 reload 时完整解析，按键热路径只读不可变快照。
