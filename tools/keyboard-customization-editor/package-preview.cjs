// Builds an offline distribution from tracked source and examples.
const fs = require('node:fs');
const path = require('node:path');
const crypto = require('node:crypto');
const zip = require('./vendor/zip-no-worker.min.js');
const root = path.resolve(__dirname, '../..');
const output = path.resolve(process.argv[2] || path.join(root, 'outputs/keyboard-customization-editor-preview'));
...
fs.cpSync(path.join(root,'examples/keyboard-customization'),path.join(output,'templates'),{recursive:true});
fs.copyFileSync(path.join(root,'scripts/package-keyboard-customization.ps1'),path.join(output,'templates/package-keyboard-customization.ps1'));
...
(async()=>{
  const packages = path.join(output, 'templates/packages');
  fs.mkdirSync(packages, { recursive: true });
  for (const [directory, name] of [['skin-soft-blue', 'soft-blue.sy-skin'], ['structure-standard', 'standard.sy-layout']]) {
    const source = path.join(root, 'examples/keyboard-customization', directory);
    const templateWriter = new zip.ZipWriter(new zip.Uint8ArrayWriter(), { useWebWorkers: false });
    for (const file of files(source).sort()) {
      await templateWriter.add(path.relative(source, file).replaceAll('\\', '/'), new zip.Uint8ArrayReader(fs.readFileSync(file)));
    }
    fs.writeFileSync(path.join(packages, name), await templateWriter.close());
  }
  ...
  try {
    ...
    if(!packaged.some(entry=>entry.filename==='使用说明.md') || !packaged.some(entry=>entry.filename==='zip.js-LICENSE.txt')) throw new Error('Missing distribution documentation');
