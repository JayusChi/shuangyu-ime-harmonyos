...
**0.13.0 APP_BUILT_AND_VERIFIED（2026-09-15）** — 当前源码已生成正式签名 APP，版本 `0.13.0 / 13000000 / build 1`，交付 `artifacts/0.13.0/ShuangYuIME-0.13.0-13000000-release-signed.app`。双 ABI、clean Release、签名、版本与资源门禁通过，正式 Profile 的皮肤共享目录授权匹配，升级身份与 0.12.0 一致。皮肤工坊入口和编辑导入功能沿用打包前的双端模拟器验收；本轮未重新安装或上传。详见 `outputs/release-0.13.0/RELEASE_READINESS.md`。下方为历史版本记录。

**0.12.0 CUSTOMER_APP_READY（2026-09-14）** — 正式客户APP已生成，`versionCode=12000000`、`buildVersion=1`，含下方九项反馈验收后的全部修复。交付为 `artifacts/0.12.0/ShuangYuIME-0.12.0-12000000-release-signed.app`，25,015,019 bytes，SHA-256 `0E23F5BBB4248D53C7F4983A951B164D18AE3A12DF6B2DBDD2593BDB3BCBDF4F`。APP/Profile/HAP正式签名、与0.11.0升级身份、权限、资源及双ABI门禁通过；与已验收HAP比对，ArkTS字节码、Native和资源完全一致，仅版本元数据变化。本轮未上传AGC或重新安装0.12.0，功能验收承接当天748项ArkTS、31项UI检查及双拼专项326项Rust结果。发布记录见 `outputs/release-0.12.0/RELEASE_READINESS.md`。下方产物和版本描述均为历史记录。

**九项反馈 PHONE_AND_COMPUTER_EMULATOR_PASS（2026-09-14）** — 两端实测补齐回删、右边距、自适应字号、回车原码、实体方括号翻页、浮动尺寸/阴影及两条双拼需求。手机慢滑仍误删的问题本轮补修：6 vp竖向意图停止长按/连删计时器，达到完整阈值后执行上下滑；键盘根保存尾随点击拦截。748项ArkTS、31项UI证据、两端无清数据覆盖安装、正式Release门禁通过。最新正式HAP为 `entry/build/release/outputs/default/entry-default-signed.hap`，93,338,611 bytes，SHA-256 `BDFE9196B990020F7C5608565020A04C1394EF8ECC0CE220ABAF82D046109204`。版本0.11.0；客户APP仍为旧产物。手机47项设置一致；电脑保留23→24升级、默认右边距10及旧候选兼容值auto→bar，其余原有字段恢复；临时固顶词条已移除。完整边界和证据见 `docs/evidence/2026-09-14-feedback-reacceptance/README.md`。下方为历史阶段记录。

**小鹤双拼整句排序与固定二字简拼 HOST_VALIDATED / PHONE_EMULATOR_PASS / RELEASE_HAP_BUILT（2026-09-14）** — `hfyzxiwh` 已核对为 `hen you xi wang`，外部 Google 拼音首选“很有希望”。修正完整小鹤音节路径中稀有长词获得过高长度奖励的问题，正式词库首选恢复“很有希望”，保留相近词频精度。我的词库二字 `#固` 规则可约束句内不能组成声韵音节的两码对齐片段，如 `woysupuurufa → 我用双拼输入法`；合法音节和跨位片段不替换，保存、修改、删除与重启复用原管理机制。相关 Rust 326 项、ArkTS 744 项、严格 Clippy、rustfmt、双 ABI Native PASS；手机模拟器虚拟键盘及实体事件模式上屏通过，47 项设置恢复一致，验收词条已移除。版本保持 0.11.0；正式 release 签名 HAP 为 `entry/build/release/outputs/default/entry-default-signed.hap`，93,331,808 bytes，SHA-256 `443141E71E32956E6A69D77A6F6ED5CA86B67AEDA74D10AFE33E68BE6FAD155B`，实包门禁 PASS。未重建客户 APP，真实外设和客户宿主未新增验收。详见 `docs/evidence/2026-09-14-shuangpin-fixed-words/README.md`。下方 HAP/APP 身份为历史产物。

**浮动候选尺寸与阴影 HOST_VALIDATED / PHONE_EMULATOR_PASS / RELEASE_HAP_BUILT（2026-09-14）** — 候选内容改为窗口完成尺寸/位置调整后发布，连续输入丢弃过期更新，同一可见组合回删不收窄；行高随字号固定，旧展示快照不能误提交新候选。白色圆角卡片、灰色选中项、浅深色阴影及密度换算已接通。744 项 ArkTS 通过；手机模拟器虚拟/实体模式 `k→kn→回删→选词` 和深色实图通过，两模式卡片均保持 196 px 高，宽度 718→812→812 px；测试前后 47 项设置一致。正式 release HAP 为 `entry/build/release/outputs/default/entry-default-signed.hap`，93,311,843 bytes，SHA-256 `1305D627C70FD62461AA1CC4ABCBD617018DCE43AA6B35081BFCD6DE92368F3E`。版本保持 0.11.0，未重新构建客户 APP；真实外设和客户宿主仍待验收。详见 `docs/evidence/2026-09-14-floating-candidate/README.md`。下方 HAP/APP 身份均为历史产物。

**键盘字号、回车编码和实体翻页 HOST_VALIDATED / RELEASE_HAP_BUILT（2026-09-14）** — 自适应默认字号与外观增减项已复核，平板按逻辑屏幕尺寸放大；有编码时实体主／小键盘回车和虚拟点按统一只上屏原始编码，空组合才执行输入框动作。实体 `[`／`]` 翻页已覆盖固定／浮动位置和快速输入队列，修正无 Unicode 驱动将 Shift＋方括号误当翻页的问题。737 项 ArkTS 全量通过，含此前候选回删／右边距回归；clean release 产品签名 HAP 与资源门禁通过。HAP 为 `entry/build/release/outputs/default/entry-default-signed.hap`，93,302,407 bytes，SHA-256 `F7304E554E292EDDFA56E820B8D7303BFFC26B804AF3382373F2198B4A1F324A`。版本保持 0.11.0；本轮未重新构建正式 APP、安装到设备或进行真实 USB／蓝牙键盘验收。详见 `docs/evidence/2026-09-14-keyboard-enter-paging/README.md`。下方 9 月 11 日 APP 是历史产物，不包含本轮回车行为变更。
